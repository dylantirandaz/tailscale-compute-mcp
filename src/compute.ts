...
END { printf "\"" }`;
  ...
  return [
    ...
    // The child must lead its own process group so the watchdog, cancellation,
    // and liveness checks can signal the whole tree. `set -m` only works where
    // /bin/sh supports job control without a terminal (bash, zsh); dash turns
    // job control off without a tty, so use setsid where it exists instead.
    `if command -v setsid >/dev/null 2>&1; then`,
    `  launch_in_own_process_group() { exec setsid "$@"; }`,
    "else",
    `  set -m`,
    `  launch_in_own_process_group() { exec "$@"; }`,
    `fi`,
    `if [ -f "$input_path" ]; then`,
    `  launch_in_own_process_group ${command} < "$input_path" > "$stdout_path" 2> "$stderr_path" &`,
    "else",
    `  launch_in_own_process_group ${command} </dev/null > "$stdout_path" 2> "$stderr_path" &`,
    "fi",
    "child_pid=$!",
    `memory_events_path=`,
